...
// 流星对话框
var dialogForm=[
    {"slng":127.8,"slat":39,"elng":127.8,"elat":39,"type":"start"}, // "top":/ 0
    ...
    {"slng":90,"slat":22,"elng":90,"elat":22,"type":"end"},//          4
    {"slng":126,"slat":23,"elng":126,"elat":23,"type":"start"} // "btmR":    5
...
infoObj.prototype.getPointPos=function(){
    if(this.lat>36){
        ...
        if(this.lng>=118){
            this.pos=5;
        }else if(this.lng>100&&this.lng<118){
            this.pos=4;
        }else{
            this.pos=3;
        }
...
infoObj.prototype.drawMeteor = function (x, y) {
    ...
    if (this.meteorTimer < meteorShow) { // 画流星
        ...
        writeTextOnCanvas(this.msg,this.pos);
    }
};
...
// canvas显示文字 [canvas，行高，行字符数，开始x，开始y，类型，文字]
function writeTextOnCanvas_old(ctx, lh, rw, startx, starty,type, text) {
    var lineheight = lh;
...
// canvas显示文字 [文字,位置]
function writeTextOnCanvas(text,pos) {
    for(var i=0;i<6;i++){
        var aid="#newsInfo_"+i;
        if(i==pos){
            $(aid).text(text);
