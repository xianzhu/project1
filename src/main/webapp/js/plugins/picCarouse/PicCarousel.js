	...
	var PicCarousel = (function()
	{
		...
		function PicCarousel(element,options){
			...
			window.onresize=function(){
				window.clearInterval(pic.timer);

				var tdelay=pic.delay;
				//setTimeout(function(){
					var dom=document.getElementById("test_category_row");
					var w=dom.offsetWidth;
					var h=dom.offsetHeight;
					//console.log(pic);
					//console.log(pic.settings);

					pic.settings.width=w*0.95;
					pic.settings.posterWidth=w*0.618;
					//console.log(pic.settings);
					//pic.init();

					pic.posterItems.each(function(){
						var w=pic.settings.posterWidth;
						...
						//var posterImgs=$(this).find("img");
						//posterImgs.each(function(){
						//	var iw=w*0.3;
						//	var img=$(this);
						//	$(this).css({
						//		width:iw,
		...
		PicCarousel.prototype = {
			...
			carouseRotate:function(dir){
				//console.log("carouseRotate");
				var me = this;
				var zIndexArr = [];
				if(dir === "left"){
					//console.log(me.posterItemMain);
					currentImg=(currentImg+1)%(me.posterItems.size());
					me.posterItems.each(function(){
						var self = $(this),
							prev = self.prev().get(0)?self.prev():me.posterLastItem,
							//width = prev.width(), // modify zl
							//height = prev.height(),
				...
				//console.log(select,', ',textArray[select]);
				//var dom=document.getElementById("currentText");
				////console.log(dom);
				//dom.innerHTML=cateGoriesList[currentImg].name;
				//$("#currentText").innerHTML=textArray[select];
			},

			//设置剩余的帧的位置关系
			setPosterPost:function(){
				//console.log("setPosterPost");
				var me = this;
